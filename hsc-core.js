...
export class HSCEngine {
    ...
    addUser(n, u, p, r) {
        const id = (r === 'admin' ? 'S-' : 'A-') + Math.floor(Math.random() * 9999);
        this.db.users.push({ id, name: n, username: u, password: p, role: r, finalGrade: "N/A" });
        this.sync();
    }
    ...
    editAssignment(id, title, points, link, dueDate) {
        ...
        if (task) {
            ...
            // Update all student grades in case point weight changed
    ...
    calculateFinalGrade(studentId) {
        ...
        const earned = graded.reduce((acc, curr) => acc + curr.score, 0);
